...
// 部分的事前レンダリングの素晴らしい点は、コードを変更することなく使用できることです。ルートの動的な部分をサスペンスでラップしている限り、Next.js はルートのどの部分が静的でどの部分が動的であるかを認識します。
// これは、コードを変更することなく、パフォーマンスを向上させることができることを意味します。 
// /app/dashboard/layout.tsxで、PPRを有効化しているのでこのページはPPRが適用される
// つまり、このページは静的な部分と動的な部分があるが、動的な部分はサスペンスでラップしているので、Next.js はルートのどの部分が静的でどの部分が動的であるかを認識します。
// そのため、動的な部分はサスペンスでラップしている限り、パフォーマンスを向上させることができる
